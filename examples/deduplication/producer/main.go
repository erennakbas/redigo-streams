...
func main() {
	...
	// Create publisher
	publisherConfig := redigo.DefaultPublisherConfig(redisURL)
	client, err := redigo.NewPublisherOnly(publisherConfig)
	if err != nil {
...
func runDeduplicationDemo(ctx context.Context, client *redigo.Client) {
...
func demonstrateRegularPublish(ctx context.Context, client *redigo.Client) {
...
func demonstrateContentDeduplication(ctx context.Context, client *redigo.Client) {
...
func demonstrateIdempotencyKey(ctx context.Context, client *redigo.Client) {
...
func demonstrateBusinessLogic(ctx context.Context, client *redigo.Client) {
	...
	for i := 1; i <= 3; i++ {
		...
		businessHash := redigo.GenerateBusinessLogicHash("user.events", userID, action)
...
func demonstrateContextBasedDeduplication(ctx context.Context, client *redigo.Client) {
	...
	for _, context := range contexts {
		...
		contextHash := redigo.GenerateBusinessLogicHash("user.events", "user-multi-context", "signup", context)
